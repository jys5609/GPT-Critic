...
class BLEUScorer(object):
    ...
    def score_point(self, hyp, refs):
        ...
        hyp = hyp.split()
        refs = [ref[0].split() for ref in refs]
...
class MultiWozEvaluator(object):
    ...
    def training_data_metric(self, data, tokenizer, bleu=False):
        if bleu:
            bleu = self.bleu_metric_datapoint(data, self.reader.dev, tokenizer)

        successes, matches, scores = self.training_data_eval(data)

        return successes, matches, scores
    ...
    def _get_metric_results(self, data, domain='all', file_list=None):
        ...
        act_precision, act_recall, act_f1 = self.aspn_eval(data, file_list)
        ...
        if dial_num:
            ...
            if cfg.bspn_mode == 'bspn' or cfg.enable_dst:
                logging.info('[ACT] precision:%2.2f recall: %2.2f f1: %2.2f' % (act_precision, act_recall, act_f1))
    ...
    def eval_bleu_datapoint(self, data_point, eval_data):
        gen, truth = [], []
        for row in eval_data:
            gen.append(data_point['resp_gen'])
    ...
    def bleu_metric_datapoint(self, data, eval_data, tokenizer):
        gen, truth = [], []
        scores = []
        ...
        for dialog in eval_data:
            for turn in dialog:
                ...
                truth.append(tokenizer.decode(turn['resp']))
        ...
        for dialog in tqdm(data):
            dialog_score = []
            for turn in dialog:
                sc = self.bleu_scorer.score_point(tokenizer.decode(turn['resp']), wrap_truth)
                dialog_score.append(sc)
            scores.append(dialog_score)
            # import pdb;
            # pdb.set_trace()

        # for hyp in gen:
        #     sc = self.bleu_scorer.score_point(hyp, wrap_truth)
        #     scores.append(sc)
    ...
    def bleu_metric(self, data, eval_dial_list=None):
        ...
        for row in data:
            ...
            # truth.append(row['resp'])
            gen.append(self.reader.tokenizer.encode(row['resp_gen']))
            truth.append(self.reader.tokenizer.encode(row['resp']))
            # print (row['resp_gen'])
            # print (row['resp'])
    ...
    def training_data_eval(self, data):
        successes, matches, cum_scores = [], [], []

        for dial in data:
            for idx in range(len(dial)):
                dial[idx]['resp_gen'] = dial[idx]['resp']
                dial[idx]['bspn_gen'] = dial[idx]['bspn']
                dial[idx]['bsdx_gen'] = dial[idx]['bsdx']
                dial[idx]['aspn_gen'] = dial[idx]['aspn']
                dial[idx]['dspn_gen'] = dial[idx]['dspn']
            ...
            success, match, cum_score = self.temporal_eval(results, cum_score=True)
            successes.append(success)
            matches.append(match)
            cum_scores.append(cum_score)
        return successes, matches, cum_scores

    def temporal_eval(self, data, eval_dial_list = None, same_eval_as_cambridge=False, cum_score=False):
        dials = self.pack_dial(data)
        counts = {}
        ...
        for dial_id in dials:
            ...
            if cum_score:
                ...
                for i in range(len(dial)):
                    success, match, stats, counts = self._evaluateGeneratedDialogue(copy.deepcopy(dial[:i + 1]), goal,
                                                                                    reqs, counts, soft_acc=True,
                                                                                    same_eval_as_cambridge=same_eval_as_cambridge)
                    cum_score.append((success + match) * 0.5)

                return success, match, cum_score

            else:
                success, match, stats, counts = self._evaluateGeneratedDialogue(dial, goal, reqs, counts, soft_acc=True,
                                                                        same_eval_as_cambridge=same_eval_as_cambridge)
                score = (success + match) * 0.5
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, real_requestables, counts,
        ...
        for t, turn in enumerate(dialog):
            if t == 0:
                # if turn['user'] == '' and turn['resp_gen'] == '':
                continue

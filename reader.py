...
class _ReaderBase(object):
    ...
    def _construct_transition_mini_batch(self, data, state_until_db=False):
        all_batches = []
        batch = []
        ...
        for dial in data:
            ...
            for i in range(len(dial)):
                transition = {}
                if state_until_db:
                    ### Construct current step information ###
                    ### transition 's' / 'sa' / 'sampled_a_list'
                    for cell in cell_list:
                        if cell in ['user', 'bspn', 'db']:
                            contexts.extend(dial[i][cell])
                        elif cell == 'aspn':
                            transition['s'] = deepcopy(contexts)
                            contexts.extend(dial[i][cell])
                        elif cell == 'resp':
                            contexts.extend(dial[i][cell])
                            transition['sa'] = deepcopy(contexts)
                        else:
                            assert NotImplementedError

                    ### Construct next step information ###
                    ### transition 'next_s' / 'next_sa' / 'sampled_next_a_list'
                    if i != len(dial) - 1:
                        next_s = deepcopy(contexts)
                        next_sa = deepcopy(contexts)
                        for cell in cell_list:
                            next_sa.extend(dial[i+1][cell])
                            if cell in ['user', 'bspn', 'db']:
                                next_s.extend(dial[i+1][cell])
                        transition['terminal'] = 0
                        transition['next_s'] = next_s
                        transition['next_sa'] = next_sa
                        transition['r'] = dial[i]['reward']
                        transition['turn_domain'] = dial[i]['turn_domain']
                    else:
                        transition['terminal'] = 1
                        transition['next_s'] = [cfg.pad_id]
                        transition['next_sa'] = [cfg.pad_id]
                        transition['r'] = dial[i]['reward']
                        transition['turn_domain'] = dial[i]['turn_domain']
                else:
                    assert NotImplementedError
    ...
    def get_transition_batches(self, set_name, scores, state_until_db=False):
        """
        Returns : List of transition tuples [(s,a,r,s'a')]
        """
        log_str = ''
        name_to_set = {'train': self.train, 'test': self.test, 'dev': self.dev}
        dial = name_to_set[set_name]
        ...
        #     dial[idx][-1]['reward'] = scores[idx]

        for idx in range(len(dial)):
            for i in range(len(dial[idx])):
                if i == 0:
                    dial[idx][i]['reward'] = scores[idx][i]
                else:
                    dial[idx][i]['reward'] = scores[idx][i] - scores[idx][i-1]
        ...
        for k in turn_bucket:
            if set_name != 'test' and k == 1 or k >= 17:
                continue
            batches = self._construct_transition_mini_batch(turn_bucket[k], state_until_db=state_until_db)
    ...
    def get_batches_with_score(self, set_name, scores):
        global dia_count
        log_str = ''
        name_to_set = {'train': self.train, 'test': self.test, 'dev': self.dev}
        dial = name_to_set[set_name]
    ...
    def get_batches(self, set_name):
        ...
        for k in turn_bucket:
            if set_name != 'test' and k == 1 or k >= 17:
                continue
            batches = self._construct_mini_batch(turn_bucket[k])
            # transition_batches = self._construct_transition_mini_batch(turn_bucket[k])
            log_str += "turn num:%d, dial num: %d, batch num: %d last batch len: %d\n" % (
                k, len(turn_bucket[k]), len(batches), len(batches[-1]))
            # print("turn num:%d, dial num:v%d, batch num: %d, "%(k, len(turn_bucket[k]), len(batches)))
            num_training_steps += k * len(batches)
...
class MultiWozReader(_ReaderBase):
    ...
    def bspan_to_PointerVector(self, bspan, turn_domain):
    ...
    def dt_convert_turn_eval(self, turn, pv_turn, first_turn=False):
        """
        ...
        if cfg.use_true_curr_bspn:
            ...
            context_list = ['return-to-go', 'user']
    ...
    def convert_turn_advantage_eval(self, turn, pv_turn, first_turn=False):
        inputs = {}
        context_list = ['user', 'bspn', 'db', 'aspn', 'resp']
        state_list = ['user', 'bspn', 'db']
        ...
        if first_turn:
            ...
            inputs['context'] = pv_turn['context'] + context
            inputs['state'] = pv_turn['context'] + state
            inputs['labels'] = pv_turn['context'] + context

        # if len(inputs['context']) > 900:
        #     inputs['context'] = inputs['context'][-900:]
        #
        # if len(inputs['state']) > 900:
        #     inputs['state'] = inputs['state'][-900:]
    ...
    def convert_turn_eval(self, turn, pv_turn, first_turn=False):
        ...
        if first_turn:
            ...
            # context = self.tokenizer.encode('<success>')
            for c in context_list:
                context += turn[c]
    ...
    def convert_policy_eval_batch_session(self, dial_batch):
        '''
        Returns: {(s,a,r,s', {a}_N, {a'}_N)}
        '''
        inputs = {}
        sas = []
        eos_sas = []
        ...
        for idx, dial in enumerate(dial_batch):
            sas.append(dial['sa'])
            next_sas.append(dial['next_sa'])
            next_ss.append(dial['next_s'])
            rs.append(dial['r'])
            eos_sas.append(len(dial['sa'])-1)
            eos_next_sas.append(len(dial['next_sa'])-1)
            eos_next_ss.append(len(dial['next_s'])-1)
            terminals.append(dial['terminal'])
            turn_domains.append(dial['turn_domain'])

        inputs['sa_np'], inputs['sa_lengths'] = utils.padSeqs_gpt(sas, cfg.pad_id)
        inputs['next_s_np'], inputs['next_s_length'] = utils.padSeqs_gpt(next_ss, cfg.pad_id)
        inputs['next_sa_np'], inputs['next_sa_length'] = utils.padSeqs_gpt(next_sas, cfg.pad_id)

        inputs['r'] = rs
        inputs['eos_sa'] = eos_sas
        inputs['eos_next_sa'] = eos_next_sas
        inputs['eos_next_s'] = eos_next_ss
        inputs['terminal'] = terminals
        inputs['turn_domain'] = turn_domains

        return inputs

    def convert_batch_session_with_advantage(self, dial_batch):
        inputs = {}
        contexts = []
        advantage_weights = []
        cell_list = ['user', 'bspn', 'db', 'aspn', 'resp']
        for idx, dial in enumerate(dial_batch):
            ...
            for turn_num, turn in enumerate(dial):
                for cell in cell_list:
                    ...
                    advantage_weight.extend([turn['advantage']] * len(turn[cell]))
                # start_idx = len(advantage_weight)
                # advantage_weight += turn['advantage'][start_idx:]
                # advantage_weight += [turn['advantage']] * len(context)
            #     print (turn['advantage'])
            # print (len(context), len(advantage_weight))
            assert len(context) == len(advantage_weight)
            contexts.append(context)
            advantage_weights.append(advantage_weight)

        inputs['contexts'] = contexts
        inputs['contexts_np'], inputs['lengths'] = utils.padSeqs_gpt(inputs['contexts'], cfg.pad_id)

        inputs['advantage_weights'] = advantage_weights
        inputs['advantage_weights_np'], inputs['advantage_weights_lengths'] = utils.padSeqs_gpt(inputs['advantage_weights'], 0)
        return inputs


    def dt_convert_batch_session(self, dial_batch):
        """
        ...
        maskings = []
        scores = []
        cell_list = ['return-to-go', 'user', 'bspn', 'db', 'aspn', 'resp']
        for idx, dial in enumerate(dial_batch):
            context = []
            masking = []
            for turn_num, turn in enumerate(dial):
                for cell in cell_list:
                    context.extend(turn[cell])
                    if cell in ['bspn', 'aspn', 'resp']:
                        masking.extend([1 for _ in range(len(turn[cell]))])
                    else:
                        masking.extend([0 for _ in range(len(turn[cell]))])

            assert len(context) == len(masking)
            contexts.append(context)
            maskings.append(masking)
            # scores.append(dial[-1]['score'])
        ...
        for i in range(len(maskings)):
            if max(inputs['lengths']) > 1024:
                if len(maskings[i]) > 1024:
                    maskings[i] = maskings[i][-1024:]
                else:
                    while len(maskings[i]) < 1024:
                        maskings[i].append(0)

            else:
                while len(maskings[i]) < max(inputs['lengths']):
                    maskings[i].append(0)

        inputs['maskings'] = maskings
        inputs['maskings_np'] = np.array(maskings)

        return inputs


    def convert_batch_session(self, dial_batch, weighted_bc=False):
        """
        convert the whole session for training
        concat [U_0, B_0, A_0, R_0, ... , U_n, B_n, A_n, R_n]
    ...
    def wrap_result_lm(self, result_dict, eos_syntax=None):
        ...
        field = ['dial_id', 'turn_num', 'user', 'bspn_gen', 'bsdx', 'resp_gen', 'resp', 'aspn_gen', 'aspn',
                     'dspn_gen', 'dspn', 'bspn', 'pointer', 'db', 'turn_domain']
